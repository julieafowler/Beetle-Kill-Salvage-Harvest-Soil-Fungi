...
#'   - model_results_MPB.csv (from Greenhouse_Bayesian_Analysis_2FactorMPBStatus.R)
...
create_full_coefficient_table <- function(results_df, model_name = "Model") {
  ...
  #' @param model_name Name for labeling ("Land-Use History" or "MPB")
  ...
  full_table <- results_df %>%
    mutate(
      ...
      predictor_clean = case_when(
        ...
        str_detect(predictor, "MPB_Impacted") ~ 
          str_replace(predictor, "MPB_Impacted", "MPB: "),
...
create_comparison_table <- function(table_landuse, table_mpb) {
  #' Create side-by-side comparison of Land-Use History vs MPB models
  #' 
  ...
  cat("  Variables selected only in MPB:", 
      sum(comparison$selected_in == "MPB Only"), "\n")
...
generate_coefficient_tables <- function(results_landuse, results_mpb, df = NULL,
  ...
  #' @param results_mpb Results from 2-level MPB analysis
  ...
  full_mpb <- create_full_coefficient_table(results_mpb, "MPB Status")
...
results_mpb <- read.csv("model_results_MPB.csv")
# 
