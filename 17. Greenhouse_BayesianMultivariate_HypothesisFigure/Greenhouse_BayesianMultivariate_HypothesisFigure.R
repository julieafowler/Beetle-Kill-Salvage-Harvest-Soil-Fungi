...
## MPB vs Non-MPB effects (2-level model)
...
## General MPB vs. Non-MPB 
## The coefficient MPB_ImpactedMPB represents the difference: MPB - Non_MPB. So testing < 0 asks "Is MPB worse than Non-MPB?"
...
for (outcome in outcomes) {
  ...
  # Test if MPB effect is negative (i.e., MPB < Non-MPB)
...
create_hypothesis_figure <- function(output_file = "mv_MI_fig_hypothesis_comparison.pdf") {
  ...
  # Red (Non-MPB vs MPB) - BOTTOM position (y_offset = -0.15)
  # Signs flipped so positive = Non-MPB advantage
  ...
  all_results <- bind_rows(landuse_results, mpb_results) %>%
    mutate(
      ...
      comparison = factor(comparison, levels = c("1stPreMPB vs 2ndPostMPB", "Non-MPB vs MPB")),
  ...
  fig <- ggplot(all_results, aes(x = estimate, y = y_pos, color = comparison)) +
    ...
    labs(
      ...
      y = "",
      caption = "Filled points indicate P > 95%. Significance: * P>95%, ** P>99%, *** P>99.9%\nPositive values indicate advantage for 1stPreMPB (blue) or Non-MPB (red) soils."
    ) +
...
create_hypothesis_figure_annotated <- function(output_file = "mv_MI_fig_hypothesis_annotated.pdf") {
  ...
  # Red (Non-MPB vs MPB) - BOTTOM position (values already flipped)
  mpb_results <- data.frame(
    outcome = c("Height", "Canopy Width", "Aboveground\nBiomass", 
                "Root Mass", "Root:Shoot\nRatio"),
    outcome_num = c(5, 4, 3, 2, 1),
