...
## Two versions: Land-Use History (6-level treatment), MPB Status (2-level treatment)
...
prepare_mv_datasets <- function(df, soil_pca_result, m = M_IMPUTATIONS) {
  ...
  vars_to_use <- c(outcomes, "Treatment_LandUseHistory", "MPB_Impacted",
  ...
  df_sub$MPB_Impacted <- factor(df_sub$MPB_Impacted)
  df_sub$MPB_Impacted <- relevel(df_sub$MPB_Impacted, ref = "Non_MPB")
...
fit_mv_model_mpb <- function(data, imp_num = 1, verbose = TRUE) {
  # Fit MPB Status multivariate model to one imputed dataset
  
  if (verbose) {
    cat("  Fitting MPB Status model (imputation", imp_num, ")...\n")
  }
  
  # Define formulas - all Gaussian to allow rescor
  bf_height <- bf(Height ~ MPB_Impacted)  # No Soil_PC2
  bf_width <- bf(MeanCanWidth ~ MPB_Impacted)  # No Soil_PC2
  bf_biomass <- bf(log_AbovegroundBiomass ~ MPB_Impacted + Soil_PC2)  # Soil_PC2 selected
  bf_root <- bf(log_RootDryMass ~ MPB_Impacted)  # No extras
  bf_ratio <- bf(log_RootToShootRatio ~ MPB_Impacted + Soil_PC2 + 
...
test_pooled_hypotheses <- function(fits_landuse, fits_mpb) {
  ...
  #' @param fits_mpb List of MPB model fits
  ...
  # --- MPB Hypotheses ---
  cat("MPB Model (pooled across", m, "imputations):\n")
  cat("--------------------------------------------\n")
  
  # H3: MPB effect equal on Height vs Width?
  h3 <- pool_hypothesis(
    fits_mpb,
    "Height_MPB_ImpactedMPB = MeanCanWidth_MPB_ImpactedMPB",
    "H3: MPB effect equal on Height vs Width"
  )
  if (!is.null(h3)) {
    cat("H3:", h3$name, "\n")
  ...
  # H4: MPB effect stronger on AbovegroundBiomass than RootDryMass?
  h4 <- pool_hypothesis(
    fits_mpb,
    "logAbovegroundBiomass_MPB_ImpactedMPB < logRootDryMass_MPB_ImpactedMPB",
    "H4: MPB reduces Aboveground more than Root"
  )
  if (!is.null(h4)) {
    cat("H4:", h4$name, "\n")
...
test_custom_hypothesis <- function(fits_list, hypothesis_string, hypothesis_name = "Custom") {
  ...
  #' 
  #' @examples
  #' # Test if Height effect = Canopy Width effect for MPB
  #' test_custom_hypothesis(
  #'   fits_mpb, 
  #'   "Height_MPB_ImpactedMPB = MeanCanWidth_MPB_ImpactedMPB",
  #'   "MPB effect equal on Height vs Width"
  #' )
  #' 
...
create_pooled_figures <- function(pooled_coef_landuse, pooled_coef_mpb,
  ...
  cor_data_mpb <- prep_cor_data(pooled_rescor_mpb$matrix, "MPB Status (2-level)")
  ...
  # -------------------------------------------------------------------------
  # FIGURE 2: MPB effects forest plot
  # -------------------------------------------------------------------------
  
  mpb_effects <- pooled_coef_mpb %>%
    filter(str_detect(predictor, "MPB_ImpactedMPB")) %>%
    mutate(
  ...
  fig2 <- ggplot(mpb_effects, aes(x = Estimate, y = outcome_clean)) +
    ...
    labs(
      title = "Effect of MPB-Affected Soils on Plant Outcomes (Pooled)",
...
run_multivariate_analysis_MI <- function(df, soil_pca_result, m = M_IMPUTATIONS) {
  ...
  # --- Step 3: Fit MPB models ---
  cat("\nSTEP 3: Fitting MPB Status models across imputations...\n")
  ...
  diag_mpb <- run_pooled_diagnostics(fits_mpb, "MPB Status")
...
## Interested in 1stPreMPB vs. 2ndPostMPB as this seems to be the big divergence within MPB statuses
...
# MPB vs Non-MPB (testing if MPB is worse, i.e., coefficient < 0)
outcomes <- c("Height", "MeanCanWidth", "logAbovegroundBiomass", 
              "logRootDryMass", "logRootToShootRatio")

for (outcome in outcomes) {
  hyp_string <- paste0(outcome, "_MPB_ImpactedMPB < 0")
  test_custom_hypothesis(mv_results_MI$fits$mpb, hyp_string, 
                         paste("MPB reduces", outcome))
}
