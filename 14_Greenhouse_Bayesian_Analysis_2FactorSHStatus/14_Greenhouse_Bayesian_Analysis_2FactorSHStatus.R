...
## This script uses Salvage Harvest Status as the dominant treatment - cannot do both Salvage Harvest Status and Land Use History (in another script) in the same model as perfectly nested but both are relevant to the study 
## Assumes have already run soil_pca_module.R and have three PCs for use here 
...
# NOTE: This version uses Salvage_Harvest_Status (2 levels) instead of Treatment_LandUseHistory
#       Salvage_Harvested = soils from SH-affected sites (1stPostMPB, 2ndPostMPB, Recent_Cut)
#       Non_Salvage_Harvested = soils from non-SH sites (1stPreMPB, 2ndPreMPB, Old_Forest)
treatments <- c("Salvage_Harvest_Status")
...
preprocess_data <- function(df, scale_vars = NULL) {
  ...
  df_processed <- df_processed %>%
    mutate(
      ...
      # Set Non_Salvage_Harvested as reference level (comparisons will be Salvage_Harvested vs Non_Salvage_Harvested)
      Salvage_Harvest_Status = relevel(Salvage_Harvest_Status, ref = "Non_Salvage_Harvested")
    )
...
# This prior encourages sparsity while allowing truly important effects through
...
build_outcome_formulas <- function(include_soil_pcs = TRUE, n_soil_pcs = 3) {
  ...
  # Base predictors (Salvage_Harvest_Status - simplified 2-level treatment)
  # NOTE: This version uses Salvage_Harvest_Status instead of Treatment_LandUseHistory
  base_preds <- c("Salvage_Harvest_Status")
  
  # NOTE: Texture NOT included - confounded with site
  
  # Soil PCs (pre-computed from soil_pca_module.R) + pH as separate predictor
...
# FINAL PREDICTOR COUNTS (SH VERSION):
#
# For each outcome:
#   - Treatment: 1 variable (creates 1 dummy variable for 2 levels: Salvage_Harvested vs Non_Salvage_Harvested)
#   - Soil PCs: 3 (PC1, PC2, PC3) + pH
#   - Rhizo fungal: 6
#   - RootTip fungal: 6
#   - Prokaryote: 1
#   - VIPs: 3-4 (outcome-specific)
#
# Total: ~20-21 predictors per outcome model (fewer dummy variables than LandUseHistory version)
...
# --- Interpretation helper for lognormal coefficients ---
interpret_lognormal_effect <- function(beta, ci_low = NULL, ci_high = NULL) {
  # For lognormal family, exp(beta) gives the multiplicative effect
  # beta > 0: outcome increases by factor of exp(beta)
  # beta < 0: outcome decreases by factor of exp(beta)
  ...
  if (fold_change > 1) {
    ...
    pct_change <- (1 - fold_change) * 100
    direction <- "decrease"
  }
  
  interpretation <- sprintf(
    "A 1-SD increase in the predictor is associated with a %.1f%% %s in the outcome (fold change: %.2f)",
    pct_change, direction, fold_change
  )
  ...
  if (!is.null(ci_low) && !is.null(ci_high)) {
    ...
    interpretation <- paste0(interpretation, 
                             sprintf("\n  95%% CI: %.2f to %.2f fold change", fc_low, fc_high))
  }
...
run_full_analysis <- function(df, soil_pca_result, m_imputations = 50) {
  ...
  cat("\nSTEP 2: Adding pre-computed soil PCs from soil_pca_module.R...\n")
  ...
  # Save individual plots - SH version
  for (out in names(coef_plots)) {
    ggsave(paste0("coef_SH_", out, ".pdf"), coef_plots[[out]], width = 10, height = 8)
    ggsave(paste0("importance_SH_", out, ".pdf"), importance_plots[[out]], width = 10, height = 8)
  }
  ...
  # Use pooled results for the final table - SH version
  results_table <- map2_dfr(pooled_results, names(pooled_results), 
                            ~create_results_table_pooled(.x, .y))
  write.csv(results_table, "model_results_SH.csv", row.names = FALSE)
  cat("  Saved: model_results_SH.csv\n")
  ...
  # Save diagnostic plots - SH version
  for (out in names(diagnostics)) {
    ggsave(paste0("ppcheck_SH_", out, ".pdf"), diagnostics[[out]]$pp_check, width = 8, height = 6)
    ggsave(paste0("trace_SH_", out, ".pdf"), diagnostics[[out]]$trace, width = 12, height = 8)
  }
  
  cat("\n")
  cat("================================================================\n")
  cat("           ANALYSIS COMPLETE (SH VERSION)                      \n")
  cat("================================================================\n")
...
#
#source("Greenhouse_Bayesian_Analysis_2FactorSHStatus.R")
results_mpb <- run_full_analysis(df, soil_results, m_imputations = 50)
#
# 3. Access results:
#    results_mpb$selection$Height          # Variable selection for Height
#    results_mpb$results_table             # Combined results table (pooled)
#    results_mpb$pooled_results$Height     # Pooled posterior draws for Height
#    results_mpb$fits$Height               # First imputation fit (for diagnostics)
#    results_mpb$diagnostics$Height        # Diagnostics for Height
