...
adonis2(mgd_ge5K_relabund.bray ~ Salvage_Harvest_Status * Soil_Depth, data = sample_tab, permutations = 999, method = "bray", by = "margin", na.action = na.omit)
...
EMF_RA_Field <- ggplot(fungal_guild_df, aes(x = Salvage_Harvest_Status, y = EMF_RA)) +
...
ggsave("Field_EMF_RA_Boxplots_bySH_wtests.pdf", EMF_RA_Field, width = 7, height = 3, units = "in")
...
PlantPath_RA_Field <- ggplot(fungal_guild_df, aes(x = Salvage_Harvest_Status, y = PlantPath_RA)) +
  facet_grid(~Soil_Depth) +
  geom_boxplot(aes(fill = Salvage_Harvest_Status), outlier.shape = NA, alpha = 0.7) +
  geom_jitter(aes(fill = Salvage_Harvest_Status), position = position_jitter(width = 0.2), size = 1.5, alpha = 0.8, shape = 21, color = "black") +
...
ggsave("Field_PlantPath_RA_Boxplots_Salvage_HarvestedImpact.pdf", PlantPath_RA_Field, width = 7, height = 3, units = "in")
...
Endophyte_RA_Field <- ggplot(fungal_guild_df, aes(x = Salvage_Harvest_Status, y = Endophyte_RA)) +
  facet_grid(~Soil_Depth) +
  geom_boxplot(aes(fill = Salvage_Harvest_Status), outlier.shape = NA, alpha = 0.7) +
  geom_jitter(aes(fill = Salvage_Harvest_Status), position = position_jitter(width = 0.2), size = 1.5, alpha = 0.8, shape = 21, color = "black") +
...
Fungal.Parasite_RA_Field <- ggplot(fungal_guild_df, aes(x = Salvage_Harvest_Status, y = Fungal.Parasite_RA)) +
  facet_grid(~Soil_Depth) +
  geom_boxplot(aes(fill = Salvage_Harvest_Status), outlier.shape = NA, alpha = 0.7) +
  geom_jitter(aes(fill = Salvage_Harvest_Status), position = position_jitter(width = 0.2), size = 1.5, alpha = 0.8, shape = 21, color = "black") +
...
## 1) Salvage Harvest
this_meta <- subset(meta, !is.na(Salvage_Harvest_Status))
...
maaslin3(
  ...
  output          = "maaslin3_ASVs_SH",
  formula         = ~ Salvage_Harvest_Status + ReadDepth,
...
res <- read_tsv("maaslin3_ASVs_SH/all_results.tsv", show_col_types = FALSE)

## Feature abundance 

# 2) Keep the predictor and model to visualize
metadatum <- "Salvage_Harvested"   # <-- edit if needed
df <- res %>%
...
# 3) Plot
ASVs_SH_volcano_abundance <-  EnhancedVolcano(
  tt,
  lab = rownames(tt),
  x = "log2FC",
  y = "q_plot",
...
ASVs_SH_volcano_abundance

ggsave("ASVs_SH_volcano_abundance.pdf", ASVs_SH_volcano_abundance, width = 8, height = 7, units = "in")
...
ASVs_SH_volcano_prevalence <- EnhancedVolcano(
  tt_prev,
...
ASVs_SH_volcano_prevalence

ggsave("ASVs_SH_volcano_prevalence.pdf", ASVs_SH_volcano_prevalence, width = 8, height = 7, units = "in")
...
# 1) Load MaAsLin3 results for this outcome
res <- read_tsv("maaslin3_guild_SH/all_results.tsv", show_col_types = FALSE)

## Feature abundance 

# 2) Keep the predictor and model you want to visualize
metadatum <- "Salvage_Harvested" 
df <- res %>%
  filter(metadata == metadatum, model == "abundance") %>%
  transmute(
    feature,
...
# 3) Plot
guild_SH_volcano_abundance <-  EnhancedVolcano(
  tt,
  lab = rownames(tt),
  x = "log2FC",
  y = "q_plot",
...
guild_SH_volcano_abundance

ggsave("guild_SH_volcano_abundance.pdf", guild_SH_volcano_abundance, width = 8, height = 7, units = "in")
...
guild_SH_volcano_prevalence <- EnhancedVolcano(
  tt_prev,
  lab = rownames(tt_prev),
  x = "log2FC",
  y = "q_plot",
...
guild_SH_volcano_prevalence

ggsave("guild_SH_volcano_prevalence.pdf", guild_SH_volcano_prevalence, width = 8, height = 7, units = "in")
