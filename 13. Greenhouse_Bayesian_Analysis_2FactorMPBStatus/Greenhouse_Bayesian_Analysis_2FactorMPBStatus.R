...
## This script uses MPB-impact status (impacted by MPB or not) as the dominant treatment - cannot do both MPB Status and Land Use History (in another script) in the same model as perfectly nested but both are relevant to the study 
...
# NOTE: This version uses MPB_Impacted (2 levels) instead of Treatment_LandUseHistory
#       MPB = soils from MPB-affected sites (1stPostMPB, 2ndPostMPB, Recent_Cut)
#       Non_MPB = soils from non-MPB sites (1stPreMPB, 2ndPreMPB, Old_Forest)
treatments <- c("MPB_Impacted")
...
preprocess_data <- function(df, scale_vars = NULL) {
  ...
  df_processed <- df_processed %>%
    mutate(
      MPB_Impacted = factor(MPB_Impacted),
      # Set Non_MPB as reference level (comparisons will be MPB vs Non_MPB)
      MPB_Impacted = relevel(MPB_Impacted, ref = "Non_MPB")
    )
...
build_outcome_formulas <- function(include_soil_pcs = TRUE, n_soil_pcs = 3) {
  ...
  # Base predictors (MPB status - simplified 2-level treatment)
  # NOTE: This version uses MPB_Impacted instead of Treatment_LandUseHistory
  base_preds <- c("MPB_Impacted")
...
# FINAL PREDICTOR COUNTS (MPB VERSION):
#
# For each outcome:
#   - Treatment: 1 variable (creates 1 dummy variable for 2 levels: MPB vs Non_MPB)
...
run_full_analysis <- function(df, soil_pca_result, m_imputations = 50) {
  ...
  # Save individual plots - MPB version
  for (out in names(coef_plots)) {
    ggsave(paste0("coef_MPB_", out, ".pdf"), coef_plots[[out]], width = 10, height = 8)
    ggsave(paste0("importance_MPB_", out, ".pdf"), importance_plots[[out]], width = 10, height = 8)
  }
  ...
  # Use pooled results for the final table - MPB version
  results_table <- map2_dfr(pooled_results, names(pooled_results), 
                            ~create_results_table_pooled(.x, .y))
  write.csv(results_table, "model_results_MPB.csv", row.names = FALSE)
  cat("  Saved: model_results_MPB.csv\n")
  ...
  # Save diagnostic plots - MPB version
  for (out in names(diagnostics)) {
    ggsave(paste0("ppcheck_MPB_", out, ".pdf"), diagnostics[[out]]$pp_check, width = 8, height = 6)
    ggsave(paste0("trace_MPB_", out, ".pdf"), diagnostics[[out]]$trace, width = 12, height = 8)
  }
  
  cat("\n")
  cat("================================================================\n")
  cat("           ANALYSIS COMPLETE (MPB VERSION)                      \n")
  cat("================================================================\n")
...
#
#source("Greenhouse_Bayesian_Analysis_2FactorMPBStatus.R")
