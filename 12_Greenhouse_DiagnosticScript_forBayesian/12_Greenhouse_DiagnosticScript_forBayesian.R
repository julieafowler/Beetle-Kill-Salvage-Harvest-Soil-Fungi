...
check_treatment_balance <- function(df) {
  ...
  if ("Salvage_Harvest_Status" %in% names(df)) {
    cat("\nSalvage_Harvest_Status levels:\n")
    print(table(df$Salvage_Harvest_Status, useNA = "ifany"))
    
    cat("\nCross-tabulation:\n")
    print(table(df$Treatment_LandUseHistory, df$Salvage_Harvest_Status, useNA = "ifany"))
  }
...
#   
#   Summary of Updates
#   Treatment variable: Using Treatment_LandUseHistory seperately in models from Salvage_Harvest_Status - perfectly nested, so testing seperately (two univariate runs)
...
# Salvage_Harvest_Status:   Perfectly nested in Treatment_LandUseHistory
